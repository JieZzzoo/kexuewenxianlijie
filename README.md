...
把一批科研 PDF 自动精读成**图文并茂的中文分析报告**并发到邮箱：每篇论文按 **正文 / 每张图 / 每张表** 三部分分析，**原图原表在上、分析在下**。纯本地 Docker 运行。
...
```mermaid
flowchart LR
  A["读 papers/*.pdf"] --> B["MinerU 云端解析"] --> P["轮询完成"]
  P --> D["逐篇循环"]
...
整条流水线 = 主工作流 **22 节点**，全在本地 n8n 容器内编排，密钥经 `.env` → `$env` 注入，工作流 JSON 不含明文密钥。
...
浏览器开 **http://localhost:5678** → 导入 `workflow/litreview-report.json` → 点 **Execute workflow** → 约 3–5 分钟后报告进邮箱。
> 只想测发信（不重跑解析）：导入 `workflow/report-send-only.json` 执行，秒级，直接读 `out/` 现有 md 拼报告发出。
...
```
workflow/               可导入的 n8n 工作流 JSON（主流程 litreview-report + 仅发送 report-send-only）
